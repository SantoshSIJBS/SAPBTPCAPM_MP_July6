...
| [Remove clashing backend session cookie](#remove-clashing-backend-session-cookie) | `REMOVE_CLASHING_BACKEND_SESSION_COOKIE`     | When set to `true`, any backend cookie whose name matches the approuter session cookie name is intercepted server-side, preventing it from reaching the browser. Default: not set (opt-in).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| Direct external session ID usage                                               | `SVC2AR_DIRECT_EXT_SESSION_ID_USAGE`            | If `true`, the `x-approuter-ext-session-id` header is accepted without requiring the `x-approuter-ext-session-id-sig` signature header, and signature verification is skipped. Use this only in trusted environments where the consumer service manages session IDs directly and the signature overhead is not needed. Default: `false`.
...
| Session cookie name                                                            | `SESSION_COOKIE_NAME`                           | Name of the session cookie sent to the client. Default: `JSESSIONID`. **Important:** When set to a value other than `JSESSIONID`, Cloud Foundry session stickiness is disabled. In this case, the application router will always fetch session data from the external session store (e.g., Redis) instead of using in-memory cached sessions. This ensures session consistency across multiple application router instances without relying on session stickiness. When using custom session cookie names, it is strongly recommended to configure [external session management](#external-session-management) with Redis for proper session handling in multi-instance deployments. 
...
* x-approuter-access-token - header containing weak IAS app2app tokens which consumer application sends to provider approuter [Service to Application Router](#service-to-application-router) 
 - The provider approuter exchanges this token for an extended token before sending it to AI Core. 
 - Cannot be used together with x-approuter-authorization header.
...
```
The corresponding xs-app.json route is:
```
{
 "source": "^/otel(.*)",
 "target": "$1",
...
### Remove Clashing Backend Session Cookie

By default, the application router classifies backend cookies as _session cookies_ (intercepted
server-side, are not forwarded to the browser) based on the absence of `expires` or `max-age`
attributes. However, if a backend service sets a cookie with the **same name** as the approuter's
own session cookie (e.g. `JSESSIONID`) but includes an explicit `max-age`, the default heuristic
would forward it to the browser — potentially overwriting the approuter session.

To prevent this, set the `REMOVE_CLASHING_BACKEND_SESSION_COOKIE` environment variable to `true`:

```yaml
env:
  REMOVE_CLASHING_BACKEND_SESSION_COOKIE: true
```

When enabled, any backend cookie whose name matches the approuter session cookie name is always
treated as a session cookie and stored server-side, regardless of its `expires` or `max-age`
attributes. The approuter logs an info-level message when such a cookie is intercepted.
...
The application router can receive a consumer service JWT token created by the SAP Authorization and Trust Management (xsuaa) service or an OIDC access token created by Identity Authentication service and use it to access the UI and the data. The token can be passed to the application router using one of two headers:

- **x-approuter-authorization**: Contains JWT or OIDC access tokens, or Basic authentication credentials
- **x-approuter-access-token**: Contains weak IAS tokens that are exchanged for extended tokens

For more information, see [Authorization Header](#authorization-header).

### Weak Token Exchange Flow

When using the `x-approuter-access-token` header, the application router performs the following steps:

1. Receives the weak token in the `x-approuter-access-token` header
2. Validates that the token contains a `jti` (JWT ID) claim
3. Exchanges the token for an extended token using the Identity Cloud service instance
4. Replaces the original header with `x-approuter-authorization` containing the extended token

This flow is for token extension.
...
If signature-based protection is not required (for example, in a trusted network environment where the consumer service manages session IDs directly), you can set `SVC2AR_DIRECT_EXT_SESSION_ID_USAGE=true`. When enabled, the `x-approuter-ext-session-id-sig` header is not required and signature verification is skipped entirely. The `x-approuter-ext-session-id` and `x-approuter-ext-session-expiry` headers are still required. Use this option only when the network boundary between the consumer service and the application router is already secured.
