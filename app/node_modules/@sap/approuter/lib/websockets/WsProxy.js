...
function createBackendConnection(req, cb) {
  ...
  handleTokenExchange(req,function (err) {
    ...
    handleExtension(req, {}, extensions , 'beforeRequestHandlerWS', (err) => {
      ...
      let protocols;
      try {
        const routeCredentials = targetUrl.route && targetUrl.route.credentials;
        const isMtls = routeCredentials && businessServiceUtils.getGrantType(routeCredentials) === businessServiceUtils.MTLS
            && routeCredentials.cert && routeCredentials.key;
        if (isMtls) {
          targetUrl.destination.strictConnectionMode = true;
        }
        options = {
          agent: agents.get(targetUrl.protocol, buildProxyUri(targetUrl.destination), targetUrl.destination),
          headers: getHeaders(req,getServiceToken(req)),
          rejectUnauthorized: !(targetUrl.destination.strictSSL === false)
        };
        if (isMtls) {
          options.cert = routeCredentials.cert;
          options.key = routeCredentials.key;
        } else if (targetUrl.destination && targetUrl.destination.cert && targetUrl.destination.key) {
          options.cert = targetUrl.destination.cert;
          options.key = targetUrl.destination.key;
        } else if (targetUrl.destination && targetUrl.destination.forwardAuthCertificates) {
          passportUtils.getAuthCertificates(req.session, targetUrl.destination, options);
        }
        protocols = req.headers && req.headers['sec-websocket-protocol'] ? req.headers['sec-websocket-protocol'] : null;
      ...
      const openConnection = function () {
        ...
        if (req.session && !req.headers['x-approuter-authorization']) {
          outgoing.on('upgrade', function (upgradeRes) {
            const setCookieHeader = upgradeRes.headers['set-cookie'];
            if (setCookieHeader) {
              const logger = req.loggingContext.getLogger('/websockets/cookies');
              const backendCookies = cookieHandler.processCookies(setCookieHeader, logger);
              if (!setBackendCookiesEnabled(req) && backendCookies.sessionCookies.length > 0) {
                cookieHandler.storeSessionCookies(backendCookies.sessionCookies, req, null);
              }
            }
          });
        }
      ...
      processOutgoingCookies(req, options.headers, req.app.get('cookieName'), function (err) {
        if (err) {
          tracer.warning('[websockets] error processing outgoing cookies: %s', err.message);
        }
        openConnection();
      });
    });
