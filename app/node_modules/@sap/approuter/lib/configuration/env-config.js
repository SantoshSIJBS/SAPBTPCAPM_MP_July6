...
function load(workingDir) {
  ...
  envConfig.ownSapCloudService = loadOwnSapCloudService();
...
function loadOwnSapCloudService() {
  const ownSapCloudService = loadJsonVar('OWN_SAP_CLOUD_SERVICE');
  if (!ownSapCloudService) {
    return null;
  }

  getValidators().validateOwnSapCloudService(ownSapCloudService);

  const normalized = ownSapCloudService
    .map((s) => s.replace(/\./g, ''))
    .filter((s) => s.length > 0);
...
function loadGroupedConfigurations() {
  let grouped = {
    ...
    checkXForwardedHostInLoginCallback: loadJsonVar('CHECK_X_FORWARDED_HOST_IN_LOGIN_CALLBACK'),
