...
const defaultCookieName = 'JSESSIONID';
...
let _sessionCookieName = getCookieName('AR-', process.env.SESSION_COOKIE_NAME || defaultCookieName);
let _stickinessEnabled = ('USE_JSESSION_COOKIE_SUFFIX_REQUIRED' in process.env && process.env.VCAP_APPLICATION)
  ? true
  : _sessionCookieName === defaultCookieName;

// For testing only — allows overriding the cached cookie name without module reload.
exports.setSessionCookieName = function (name) {
  _sessionCookieName = name !== undefined ? name : getCookieName('AR-', process.env.SESSION_COOKIE_NAME || defaultCookieName);
  _stickinessEnabled = ('USE_JSESSION_COOKIE_SUFFIX_REQUIRED' in process.env && process.env.VCAP_APPLICATION)
    ? true
    : _sessionCookieName === defaultCookieName;
};
...
exports.getSessionCookieName = function () {
  ...
  // If process.env.SESSION_COOKIE_NAME or "AR-" cookie generated Cloud Foundry session stickiness won't work,
  // in such case Redis can be used for central session management
  return _sessionCookieName;
};

exports.isSessionStickinessEnabled = function () {
  // When USE_JSESSION_COOKIE_SUFFIX_REQUIRED is set, getCookieName() returns 'AR-<guid>'
  // which is never equal to JSESSIONID, but session stickiness is still intended.
  // In that case the suffix is CF-managed stickiness via the AR- prefix — not Redis mode.
  return _stickinessEnabled;
};
...
exports.addStateSessionCookie = async function (req, res) {
  const stateCookieName = state.getStateCookieName();
  const cookieConfig = req.routerConfig && req.routerConfig.cookies || {};
  const { SameSite :sameSite } = cookieConfig;
  ...
  const stateCookieValue = crypto.randomUUID();
  const stateValue = crypto.randomUUID();

  req.state = stateValue;                   // Public: will appear in URL
  req.preAuthSessionId = stateCookieValue;  // Secret: only in cookie

  // Store mapping from state to cookie for validation on callback
  await state.setStateToCookieMapping(stateValue, stateCookieValue, req);

  await state.setCacheStateValue(stateValue, req);

  const cookieOptions = { path: '/', httpOnly: true, secure: true, sameSite, maxAge: state.getStateTtl() };
  if (this.shouldPartitionCookie(req)) {
    cookieOptions.partitioned = true;
  }
  const stateCookie = cookie.serialize(stateCookieName, stateCookieValue, cookieOptions);
  this.setCookie(res, stateCookie);
};

exports.isRedisStoreAvailable = function (req) {
  const externalSessionStore = req.app && typeof req.app.get === 'function' && req.app.get('externalSessionStore');
  return !!(externalSessionStore && typeof externalSessionStore.checkClient === 'function' && externalSessionStore.checkClient());
};

exports.setStateOnly = function (req) {
  req.state = crypto.randomUUID();
};
...
exports.decryptCookies = function (cookies, backendName, logger) {
  ...
  if (encryptedCookie) {
    ...
    try {
      ...
      logger && logger.info('Failed to decrypt cookie with key %s and value %s: %s', encryptedCookie.key, encryptedCookie.value, err.message);
