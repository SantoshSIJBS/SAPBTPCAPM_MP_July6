    ...
    for (const key in req.html5RepoCredentials) {
      ...
      if (typeof value === 'object') {
        configuration = Object.values(value).find(
          (entry) => entry && (entry['sap.cloud.service'] === serviceName || entry['uniqueServiceName'] === serviceName)
        );

        if (configuration && configuration['sap.cloud.service']) {
          const serviceId = configuration.uaa && configuration.uaa.serviceInstanceId;
          const normalizedCloudService = configuration['sap.cloud.service'].replace(/\./g, '');
          configuration.uniqueServiceName = serviceId
            ? normalizedCloudService + '!' + serviceId
            : normalizedCloudService;
          break;
        }
      }
    }
...
async function getHTML5RepoToken(req) {
  ...
  if (!html5RepoToken) {
    ...
    await tokenUtils.loadClientCredentialsToken(req.app, headerUtils.getCorrelationId(req), html5Cred, html5RTService);
...
async function _getHTML5RepoOptions(req, path, appHostId) {
  ...
  if (req.headers && req.headers['x-html5-repo-flow']) {
    requestOptions.headers['x-html5-repo-flow'] = req.headers['x-html5-repo-flow'];
  }
...
function handleDestinations(configuration) {
  ...
  destinations.forEach((destination) =>{
    ...
    if (normalizedDestination.timeout) {
      if (isNaN(normalizedDestination.timeout)) {
        throw new Error(`Invalid destination '${normalizedDestination.name}': timeout must be a number, received '${normalizedDestination.timeout}'`);
      }
