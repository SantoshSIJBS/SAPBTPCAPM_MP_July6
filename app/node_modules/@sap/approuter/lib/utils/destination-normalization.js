...
const {getServiceCredentials} = require('./vcap-utils');
...
function normalizeDestinationProperties(destinations) {
  ...
  destinations.forEach(function (destination, i, destinations) {
    if (destinations[i].Authentication) { // if it destination from destinations service
      ...
      destinations[i].strictConnectionMode = destinations[i].strictConnectionMode === 'true' || destinations[i].strictConnectionMode === true;
      ...
      if (destinations[i].timeout) {
        if (isNaN(destinations[i].timeout)) {
          throw new Error(`Invalid destination '${destinations[i].name}': timeout must be a number, received '${destinations[i].timeout}'`);
        }
...
function adjustDestinationProperties(destinations) {
  destinations.forEach(function (destination) {
    if (destination.proxyType === 'OnPremise') {
      try {
        let credentials = getServiceCredentials({tag: 'connectivity'});
        destination['proxyHost'] = credentials.onpremise_proxy_host;
        destination['proxyPort'] = credentials.onpremise_proxy_port;
      } catch (_error) {
        throw new Error(`Destination "${destination.name}" with  ProxyType "OnPremise" but connectivity service is not bound.`);
      }
    }
...
module.exports = {
  normalizeDestinationProperties,
  adjustDestinationProperties,
