...
// 360s gives the SWR background refresh (default threshold: 30s) enough headroom
// to complete before credentials expire. Kept at 360s rather than 300s so a
// delayed background fetch (e.g. under high load) still has a safe margin.
const stdTTL = 360;
...
const SWR_DEFAULT_THRESHOLD = 30; // seconds
...
module.exports.backgroundRefreshInProgress = backgroundRefreshInProgress;
module.exports.MTLS = 'mtls';
...
function cacheBSDestinations(req, cb) {
  ...
  let swrThreshold   = getValidatedEnvInt('BS_CACHE_SWR_THRESHOLD', SWR_DEFAULT_THRESHOLD);
  let tracer = req.loggingContext.getTracer(__filename);
  ...
  if (destinationBSCredentials) {
    if (swrThreshold > 0) {
      const remaining = getRemainingTtlSeconds(req);
      const cacheKey  = getCacheKey(req);
      if (remaining !== null && remaining <= swrThreshold && !backgroundRefreshInProgress.has(cacheKey)) {
        backgroundRefreshInProgress.add(cacheKey);
        tracer.debug('SWR: background refresh triggered for ' + cacheKey + ', TTL=' + remaining + 's');
        // Extract only scalar values from req to avoid capturing the live req object
        // in the closure — prevents memory pressure and resource leaks at high TPS.
        const swrCtx = {
          tenant: req.tenant,
        ...
        setImmediate(function () {
          module.exports.fetchDestinationBSCredentials(swrCtx, tracer, function (err, result) {
            backgroundRefreshInProgress.delete(cacheKey);
            if (err) {
              tracer.error('SWR: background refresh failed for ' + cacheKey + ': ' + err.message);
              return;
            }
            let fetchedBSCredentials = result.destinationBSCredentials;
            if (swrCtx.destinationKey) {
              instanceDestinationBSCredentialsCache.set(swrCtx.destinationKey, fetchedBSCredentials, specificStdTTL);
              expiredDataMapInstance.delete(swrCtx.destinationKey);
            } else {
              tenantDestinationBSCredentialsCache.set(swrCtx.tenant, fetchedBSCredentials, specificStdTTL);
              expiredDataMapTenant.delete(swrCtx.tenant);
            }
            tracer.debug('SWR: background refresh complete for ' + cacheKey);
          });
        });
      }
    }
      ...
      if (destination.hasOwnProperty('sap.cloud.service')) {
        ...
        if (!destination.hasOwnProperty('app_host_id')) {
          ...
          if (destination.destinationId) {
            ...
            // Skip html5-apps-repo-rt as it is expected to appear multiple times in subaccount level destinations
            if (!destination.hasOwnProperty('html5-apps-repo') && serviceName !== html5RTService) {
...
function getCacheKey(req) {
  return req.destinationKey || req.tenant;
}

function getRemainingTtlSeconds(req) {
  const cache = req.destinationKey
    ? instanceDestinationBSCredentialsCache
    : tenantDestinationBSCredentialsCache;
  const expiryMs = cache.getTtl(getCacheKey(req));
  // getTtl returns undefined (key missing) or 0 (no expiry) — both mean SWR not applicable
  if (!expiryMs) {
    return null;
  }
  return Math.ceil((expiryMs - Date.now()) / 1000);
}
