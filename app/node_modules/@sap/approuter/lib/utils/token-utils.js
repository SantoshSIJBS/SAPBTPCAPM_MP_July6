...
exports.exchangeIdentityTokenByIssuer = exchangeIdentityTokenByIssuer;
...
exports.getAndLoadClientCredentialsToken = getAndLoadClientCredentialsToken;
...
function createAuthService(credentials, type) {
  ...
  if (creds.domains && typeof creds.domains === 'string') {
    creds.domains = JSON.parse(creds.domains);
  }
...
async function loadClientCredentialsToken(app, correlationId, credentials, serviceName, cb) {
  ...
  try {
    ...
    if (tokenResponseOptions.errorMessage) {
      ...
      if (cb) {return cb(tokenResponseOptions.errorMessage);}
      throw new Error(tokenResponseOptions.errorMessage);
    }
    if (!app.services) {
      app.services = {};
    }
    ...
    const errorMessage = tokenResponseOptions.errorMessage || err.message;
    tracer.error(errorMessage);
    if (cb) {return cb(errorMessage);}
    throw new Error(errorMessage);
  }
}

async function getAndLoadClientCredentialsToken(req, credentials, serviceName, tenant) {
  const correlationId = headerUtils.getCorrelationId(req);
  if (tenant) {
    const cachedTenantToken = req.app && req.app.services && req.app.services[serviceName]
      && req.app.services[serviceName][tenant] && req.app.services[serviceName][tenant].token;
    if (!cachedTenantToken || isTokenExpired(cachedTenantToken)) {
      const tokenResponse = await getClientCredentialsTokenByTenant(correlationId, tenant, credentials);
      if (!req.app.services) { req.app.services = {}; }
      if (!req.app.services[serviceName]) { req.app.services[serviceName] = {}; }
      if (!req.app.services[serviceName][tenant]) { req.app.services[serviceName][tenant] = {}; }
      req.app.services[serviceName][tenant].token = tokenResponse;
    }
    return req.app.services[serviceName][tenant].token.accessToken;
  }
  const serviceToken = req.app && req.app.services && req.app.services[serviceName] && req.app.services[serviceName].token;
  if (!serviceToken || isTokenExpired(serviceToken)) {
    await new Promise((resolve, reject) => {
      loadClientCredentialsToken(req.app, correlationId, credentials, serviceName, (err) =>
        err ? reject(new Error('Failed to load client credentials token: ' + err)) : resolve()
      );
    });
  }
  return req.app.services[serviceName].token.accessToken;
}
...
async function exchangeIdentityTokenByIssuer(tokenEncoded, tokenDecoded, correlationId, externalServiceCredentials) {
  const app_tid = tokenDecoded.app_tid;
  const issuer = tokenDecoded.ias_iss;
  if (!issuer) {
    throw new Error('Token is missing required ias_iss claim');
  }
  const clonedCredentials = _.clone(externalServiceCredentials);
  clonedCredentials.url = issuer;
  const identityService = new IdentityService(clonedCredentials,{requests: { retry: getXSSECRetryConfig()}});
  return identityService.fetchJwtBearerToken(tokenEncoded, { correlationId,  app_tid, timeout });
}

// Supported auth services: xsuaa and identity (ias)
// TokenFetchOptions: { correlationId, timeout } -> base type
// IdentityServiceTokenFetchOptions: { app_id, resource }
// XsuaaTokenFetchOptions { tenant, zid }
