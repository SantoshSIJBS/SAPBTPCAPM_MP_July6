...
function getConfigurations(req, appKey, callback) {
  ...
  getVcapServicesData(req, appKey, function (err, vcapServicesData) {
    ...
    request.get(vcapServicesData.requestOptions, function onResponse(err, res, body) {
      ...
      const invalidAppHostIds = res.headers && res.headers['x-invalid-app-host-id'];
      if (invalidAppHostIds) {
        logger && logger.warning('Application configuration is not valid. Invalid App Host IDs found - ' + invalidAppHostIds + '. Please check with business service \'' + appKey.appPrefix + '\' provider if the requested App Host IDs are valid or try to re-bind the business service');
      }
