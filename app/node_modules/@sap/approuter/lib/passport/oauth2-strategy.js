...
OAuth2Strategy.prototype.authenticate = async function (req, options) {
  ...
  if (
    ...
    const actualCookie = cookies[state.getStateCookieName()];
    ...
    const stateFound = await state.getCacheStateValue(req.query.state, req);
    if (!stateFound) {
    ...
    const expectedCookie = await state.getStateToCookieMapping(req.query.state, req);
    if (!expectedCookie) {
  ...
  if (process.env.PRESERVE_FRAGMENT !== 'false') {
    ...
    if (!cookies[redirectCookieName] || !cookies[redirectCookieName][0]) {
      logger.error('%s cookie is missing for request url %s', redirectCookieName, getMaskedCallbackURL(req.url));
      returnResponseStatus(req, 400, redirectCookieName + ' cookie is missing');
      return;
    }

    const redirectCookie = querystring.unescape(cookies[redirectCookieName][0]);
...
async function getAccessToken(req, that, callbackURL, loggingContext) {
  const logger = loggingContext.getLogger('/Auth/OAuth2');
  ...
  // Get the state cookie value for PKCE verifier lookup and cleanup
  const cookies = cookie.parse(req.headers.cookie || '');
  const sessionCookie = cookies[state.getStateCookieName()];

  const callback = passportUtils.getAccessTokenCallback(that, req);

  async function cleanupStateCache() {
    if (stateParam) {
      state.deleteCacheStateValue(stateParam);
      await state.deleteStateToCookieMapping(stateParam, req);
      if (sessionCookie) {
        state.deleteCacheStateValue(sessionCookie);
      }
    }
    if (pkceEnabled && stateParam && sessionCookie) {
      await state.deleteCodeVerifier(sessionCookie, req);
    }
    if (stateParam) {
  ...
  if (pkceEnabled && stateParam) {
    ...
    const codeVerifier = sessionCookie ? await state.getCodeVerifier(sessionCookie, req) : undefined;
