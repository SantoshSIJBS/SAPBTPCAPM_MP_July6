  ...
  refreshToken: function (app, session) {
    ...
    memoryStore.getSessionTimeout(session.id, function (err, sessionTimeout) {
      ...
      memoryStore.update(session.id, function (newSession) {
        ...
        if (newSession.jwtRefreshStarted) {
          // Guard against a stuck flag: if the instance that set jwtRefreshStarted crashed
          // before completing the refresh, the flag stays in Redis indefinitely.
          // Treat it as stale once jwtRefresh minutes + 60s have elapsed.
          const stalenessThresholdMs = toMilliseconds(jwtRefresh) + 60000;
          const isStale = newSession.jwtRefreshStartedAt
            && (Date.now() - newSession.jwtRefreshStartedAt) > stalenessThresholdMs;
          if (!isStale) {
            return;
          }
          tracer.warning('jwtRefreshStarted flag is stale for session %s (set %dms ago), resetting.',
            newSession.id, Date.now() - newSession.jwtRefreshStartedAt);
        }
        tracer.info('Updating jwtRefreshStarted to true for session %s.', newSession.id);
        newSession.jwtRefreshStarted = true;
        newSession.jwtRefreshStartedAt = Date.now();
      ...
      }, false, true); // flushToRedis=true: propagate jwtRefreshStarted to Redis immediately
    });
  }
};
...
function doRefresh(app, session) {
  ...
  passportUtils.callUaa(requestOptions, checkTimeout, function (err, uaaResponse) {
    ...
    memoryStore.update(session.id, function updateSession(newSession) {
      ...
      newSession.jwtRefreshStartedAt = undefined;
