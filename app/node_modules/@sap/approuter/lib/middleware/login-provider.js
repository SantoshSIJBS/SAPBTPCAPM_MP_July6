...
module.exports = {
  ...
  isExchangeTokenRequired: function (req) {
    ...
    // If token exchange is not required, still collect the appKey if present
    if (!isExchangeTokenRequired && req.appKey) {
      module.exports.collectAppKey(req);
    }

    return isExchangeTokenRequired;
  },

  collectAppKey: function (req) {
    const uniqueServiceName = req.destinationCredentials && req.destinationCredentials.uniqueServiceName;
    if (!uniqueServiceName || !req.appKey || !req.appKey.entireKey) {
      return;
    }
    if (req.session && req.session.user && req.session.user.businessServices
          && req.session.user.businessServices[uniqueServiceName]
          && req.session.user.businessServices[uniqueServiceName].appKeys
          && req.session.user.businessServices[uniqueServiceName].appKeys[req.appKey.entireKey]){
      return;
    }
    ...
    sessionExt.update(req.session, function (session) {
      ...
      if (!session.user.businessServices[uniqueServiceName]) {
        return;
      }
      if (!session.user.businessServices[uniqueServiceName].appKeys) {
        session.user.businessServices[uniqueServiceName].appKeys = {};
      }
      session.user.businessServices[uniqueServiceName].appKeys[req.appKey.entireKey] = true;
    });
  },
  ...
  getAuthenticator: function (req, res, cb) {
    ...
    getXSUAAOauthAuthenticator(req, async function (err, authenticator) {
      if (err) {
        return cb(err);
      }
      ...
      if (cookieUtils.shouldPartitionCookie(req) || pathUtil.isStateProtectionEnabled(req) || logoutProvider.getBackChannelLogoutEndpoint(req)) {
        try {
          await cookieUtils.addStateSessionCookie(req, res);
        } catch (err) {
          return cb(err);
        }
      }
...
function updateSession(req, iasExchangedToken, xsuaaExchangedToken) {
  ...
  sessionExt.update(req.session, function (session) {
    ...
    if (req.appKey && req.appKey.entireKey) {
      session.user.businessServices[req.destinationCredentials.uniqueServiceName].appKeys[req.appKey.entireKey] = true;
    }
  });
}