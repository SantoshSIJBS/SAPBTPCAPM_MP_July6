...
module.exports = function loginCheck(req, res, next) {
  const exchangeTokenRequired = loginProvider.isExchangeTokenRequired(req);
  if ((!loginProvider.isLoginRequired(req) && !exchangeTokenRequired) ||
      (logoutProvider.isLogoutRequest(req) && process.env.LOGOUT_WO_SESSION_TRIGGERS_LOGIN !== 'true')) {
    return next();
  } else if (exchangeTokenRequired){
