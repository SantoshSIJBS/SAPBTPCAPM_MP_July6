...
function getServiceByName(requestedServiceName, route) {
  ...
  if (services) {
    for (let serviceName in services) {
      ...
      if (service.credentials && (service.label === requestedServiceName || service.credentials['sap.cloud.service'] === requestedServiceName)) {
        ...
        service.credentials['sap.cloud.service'] && (serviceCredentials['sap.cloud.service'] = service.credentials['sap.cloud.service']);
        return serviceCredentials;
      }
    }
  }
  if (route && route.credentials && route.credentials['grant_type'] === CLIENT_CREDENTIALS) {
    const creds = route.credentials;
    const serviceCredentials = creds.uaa ? creds.uaa : creds;
    creds['grant_type'] && (serviceCredentials['grant_type'] = creds['grant_type']);
    creds['saasregistryenabled'] && (serviceCredentials['saasregistryenabled'] = creds['saasregistryenabled']);
    creds['saasregistryappname'] && (serviceCredentials['saasregistryappname'] = creds['saasregistryappname']);
    creds['sap.cloud.service'] && (serviceCredentials['sap.cloud.service'] = creds['sap.cloud.service']);
    return serviceCredentials;
  }
  return null;
}
...
async function loadClientCredentialsToken(req,businessServiceData, correlationId) {
  if (businessServiceData.loadTokenRequired) {
    const sapCloudService = businessServiceData.credentials['sap.cloud.service'] || businessServiceData.service;
    logRequestInfo(req, `cc-middleware - loading XSUAA client credentials token for service: ${sapCloudService}`);
    await tokenUtils.loadClientCredentialsToken(req.app,correlationId, businessServiceData.credentials, sapCloudService);
  }
  if (businessServiceData.iasLoadTokenRequired) {
    logRequestInfo(req, `cc-middleware - loading IAS client credentials token for service: ${businessServiceData.service}`);
...
async function exchangeClientCredentialsTokenByTenant(req, businessServiceData, correlationId) {
  if (businessServiceData.iasTokenExchangeRequired) {
    ...
    logRequestInfo(req, `cc-middleware - exchanging IAS client credentials token for service: ${businessServiceData.service}, in tenant: ${businessServiceData.tenant}`);
    const token =  await tokenUtils.getIASClientCredentialsToken(businessServiceData.iasCredentials, businessServiceData.credentials.instanceId, correlationId);
  ...
  if (businessServiceData.tokenExchangeRequired) {
    logRequestInfo(req, `cc-middleware - exchanging XSUAA client credentials token for service: ${businessServiceData.service}, in tenant: ${businessServiceData.tenant}`);
...
function getBusinessServiceData(req) {
  ...
  const serviceCredentials = getServiceByName(serviceName, route);
  ...
  const enableLazyHTML5RepoTokenExchange = process.env.ENABLE_LAZY_HTML5_TOKEN_EXCHANGE !== 'false';
  return {
    tenant: tenant,
    credentials: serviceCredentials,
    service: serviceNameInt,
    loadTokenRequired: (serviceName === CONNECTIVITY || serviceCredentials['grant_type'] === CLIENT_CREDENTIALS) && (!existingToken || tokenUtils.isTokenExpired(existingToken)),
    iasLoadTokenRequired: serviceName === CONNECTIVITY && useIASToken && (!iasExistingToken || tokenUtils.isTokenExpired(iasExistingToken)),
    tokenExchangeRequired: (serviceName !== 'html5-apps-repo' || enableLazyHTML5RepoTokenExchange) && isMultitenant && (!tenantToken || tokenUtils.isTokenExpired(tenantToken)),
    iasTokenExchangeRequired: serviceName === CONNECTIVITY && useIASToken && isMultitenant && (!iasTenantToken || tokenUtils.isTokenExpired(iasTenantToken)),
