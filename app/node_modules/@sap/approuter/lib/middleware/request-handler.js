...
module.exports = function requestHandler(req, res, next) {
  ...
  const credentials = req.internalUrl.route.credentials;
  if (serviceTag && businessServiceUtils.getGrantType(credentials) === 'user_token'){
    ...
    const serviceName = req.destinationCredentials && req.destinationCredentials.uniqueServiceName && credentials && credentials.uniqueServiceName ? credentials.uniqueServiceName : serviceTag;
    callBackend(req, res, req.session.user.businessServices[serviceName].accessToken, next);
  } else if (serviceTag && businessServiceUtils.getGrantType(credentials) === businessServiceUtils.MTLS) {
    // mTLS: no token — cert/key from binding credentials are injected in options.js
...
function onResponse(requests, requestStats, next) {
  ...
  const backendCookies = cookieHandler.processCookies(outgoingResponse.headers['set-cookie'], logger);
