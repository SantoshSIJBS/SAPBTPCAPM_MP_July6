...
const stateUtils = require('../passport/state');
const VError = require('verror').VError;

module.exports = async function passportLogin(req, res, next) {
  ...
  try {
    await loginCallbackProvider.validateXForwardedHost(req);
  } catch (err) {
    return next(err);
  }
  ...
  oauthConfig.getXSUAAOauthStrategy(req, function (err, strategy) {
    ...
    passport.authenticate(strategy.name, function (err, user) {
      ...
      req.logIn(user,oauthConfig.SESSION_OPTIONS, function (err) {
        ...
        // In no-session-stickiness mode a separate _STATE cookie was set during
        // login initiation. Delete it now that the real session cookie is issued.
        if (!cookieUtils.isSessionStickinessEnabled()) {
          cookieUtils.setCookie(res, cookieUtils.addAttributes(cookieUtils.serialize(req, stateUtils.getStateCookieName(), '', cookieOptions), req));
        }
        ...
        if (fragmentCookieName in cookies) {
