...
async function cacheAndGetCredentials(req) {
  ...
  if (!isHTML5RepoCredsExcluded(req) && serviceName) {
    await html5RepoUtils.cacheHTML5Applications(req, appKey);
    const html5Credentials = html5RepoUtils.getCredentials(serviceName, req);
    if (html5Credentials && (html5Credentials.uaa || html5Credentials.ias)) {
      req.destinationCredentials = html5Credentials;
    }
  }

  // If no credentials found, try to get from destination service
  if (!req.destinationCredentials && ((!req.disabledDestCred && serviceName) || !serviceName)) {
    await cacheBSDestinationsProm(req);
    const bsCredentials = serviceName && bsUtils.getCredentials(serviceName, true, req);
    if (bsCredentials && (bsCredentials.uaa || bsCredentials.ias)) {
      req.destinationCredentials = bsCredentials;
    }
  }

  if (!req.destinationCredentials) {
    logRequestInfo(req,`Could not find credentials for request: ${req.url}`);
  }
}
