...
class XMLAnnotationServiceAdapter {
    ...
    sync(fileCache) {
        ...
        const metadataAnnotations = (0, xml_odata_annotation_converter_1.convertDocument)(this.service.metadataFile.uri, metadataDocument);
        ...
        if (this.collectV2Annotations) {
            [this.metadata, this.v2annotations] = (0, xml_odata_annotation_converter_1.convertMetadataDocumentV2)(this.service.metadataFile.uri, metadataDocument);
        }
        else {
     ...
     */
    syncExternalService(uri, data, localFilePath) {
        ...
        const metadataAnnotations = (0, xml_odata_annotation_converter_1.convertDocument)(uri, metadataDocument);
        ...
        const metadata = (0, xml_odata_annotation_converter_1.convertMetadataDocument)(uri, metadataDocument);
    ...
    getExternalServices() {
        return Array.from(this.externalServices.entries()).map(([uri, { annotations, metadata, localFilePath }]) => {
            ...
            return {
                uri,
                localFileUri: (0, node_url_1.pathToFileURL)(localFilePath).toString(),
    ...
    addMissingMetadataReferences() {
        ...
        for (const file of this.service.annotationFiles) {
            ...
            const metadataReference = annotationFile.references.find((reference) => reference.name === metadataNamespace.name);
            if (!metadataReference) {
                const reference = (0, odata_annotation_core_types_1.createReference)(metadataNamespace.name, metadataNamespace.alias);
    ...
    getValueListReferencesForTarget(target) {
    ...
    collectValueListReferences(annotationFile) {
        ...
        for (const target of annotationFile.targets) {
            for (const annotation of target.terms) {
                ...
                const parsedTerm = (0, odata_annotation_core_1.parseIdentifier)(term);
                if (parsedTerm.name !== 'ValueListReferences') {
                    continue;
                }
                const namespace = namespaces.find((ns) => ns.alias === parsedTerm.namespaceOrAlias || ns.name === parsedTerm.namespaceOrAlias)?.name;
                if (namespace !== 'com.sap.vocabularies.Common.v1') {
                    continue;
                }
                const collection = annotation.content.find((element) => (0, odata_annotation_core_1.isElementWithName)(element, "Collection" /* Edm.Collection */));
                if ((0, odata_annotation_core_1.isElementWithName)(collection, "Collection" /* Edm.Collection */) && annotation.range) {
                    const references = collection.content
                        .map((reference) => {
                        if ((0, odata_annotation_core_1.isElementWithName)(reference, "String" /* Edm.String */)) {
                            const value = reference.content[0];
                            if (value.type === odata_annotation_core_types_1.TEXT_TYPE) {
                                return value.text;
                            }
                        }
                        return undefined;
                    ...
                    valueListReferences.push({
                        annotation,
                        uris: references,
                        location: odata_annotation_core_types_1.Location.create(annotationFile.uri, annotation.range),
...
function parseWithoutCache(content, ignoreComments = true) {
