...
exports.convertMetadataDocumentV2 = convertMetadataDocumentV2;
 ...
 */
function convertMetadataDocumentV2(uri, document) {
    const root = document.rootElement;
    if (!root) {
        return [[], []];
    }
 ...
 */
function convertSchema(schema, aliasMap, uri, metadataElements, v2annotations) {
 ...
 */
function enrichNavigationPropertyMetadata(context, element, type, props) {
    const referentialConstraints = (0, element_getters_1.getElementsWithName)("ReferentialConstraint" /* Edm.ReferentialConstraint */, element) ?? [];
    props.referentialConstraints = referentialConstraints.map((constraint) => {
        const property = (0, attribute_getters_1.getAttributeValue)("Property" /* Edm.Property */, constraint);
        const referencedProperty = (0, attribute_getters_1.getAttributeValue)("ReferencedProperty" /* Edm.ReferencedProperty */, constraint);
        return {
    ...
    const metadataElementPartner = (0, attribute_getters_1.getAttributeValue)("Partner" /* Edm.Partner */, element);
    if (metadataElementPartner) {
        props.partner = metadataElementPartner;
    }
    const metadataElementContainsTarget = (0, attribute_getters_1.getAttributeValue)("ContainsTarget" /* Edm.ContainsTarget */, element);
 ...
 * @param props - metadata element properties to mutate
 */
function enrichEntityTypeMetadata(element, props) {
...
function createMetadataElementNodeForType(context, element, type, forAction = false) {
    ...
    if (element.name === "NavigationProperty" /* Edm.NavigationProperty */) {
...
function collectV2Annotations(context, element, target) {
        ...
        if (typeof attrKey === 'string' && attrKey.startsWith('sap:')) {
            ...
            if (typeof attrValue === 'string') {
                const annotation = {
                    ...
                    value: attrValue,
                    target,
                    valueRange: (0, range_1.transformRange)(attribute.syntax.value),
                    nameRange: (0, range_1.transformRange)(attribute.syntax.key),
                    range: (0, range_1.transformRange)(attribute.position)
                };
 ...
 */
function stringToNumberWithKeywords(value, allowedKeyWords) {
    if (value) {
 ...
 */
function getMetadataElementFacets(element) {
    const facets = {};
    const isNullable = stringToBoolean((0, attribute_getters_1.getAttributeValue)("Nullable" /* Edm.Nullable */, element));
    if (isNullable !== undefined) {
        facets.isNullable = isNullable;
    }
    const maxLength = stringToNumber((0, attribute_getters_1.getAttributeValue)("MaxLength" /* Edm.MaxLength */, element));
    if (maxLength !== undefined) {
        facets.maxLength = maxLength;
    }
    const precision = stringToNumber((0, attribute_getters_1.getAttributeValue)("Precision" /* Edm.Precision */, element));
    if (precision !== undefined) {
        facets.precision = precision;
    }
    const scale = stringToNumberWithKeywords((0, attribute_getters_1.getAttributeValue)("Scale" /* Edm.Scale */, element), [
        VARIABLE_KEY_WORD,
        FLOATING_KEY_WORD
    ]);
    if (scale !== undefined) {
    ...
    const supportsUnicode = stringToBoolean((0, attribute_getters_1.getAttributeValue)("Unicode" /* Edm.Unicode */, element));
    if (supportsUnicode !== undefined) {
        facets.supportsUnicode = supportsUnicode;
    }
    const srid = stringToNumberWithKeywords((0, attribute_getters_1.getAttributeValue)("SRID" /* Edm.SRID */, element), [VARIABLE_KEY_WORD]);
    if (srid !== undefined) {
        facets.srid = srid;
    }
    const defaultValue = (0, attribute_getters_1.getAttributeValue)("DefaultValue" /* Edm.DefaultValue */, element);
    if (defaultValue) {
