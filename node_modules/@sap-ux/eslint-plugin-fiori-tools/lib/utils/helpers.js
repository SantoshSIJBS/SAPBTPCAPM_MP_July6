...
exports.createIsWindowObject = createIsWindowObject;
exports.createRememberWindow = createRememberWindow;
exports.createIsDocument = createIsDocument;
exports.createIsDocumentObject = createIsDocumentObject;
exports.createRememberDocument = createRememberDocument;
exports.buildCalleePath = buildCalleePath;
exports.getMemberAsString = getMemberAsString;
exports.createIsHistory = createIsHistory;
exports.createIsHistoryObject = createIsHistoryObject;
exports.createRememberHistory = createRememberHistory;
exports.createIsLocation = createIsLocation;
exports.createIsLocationObject = createIsLocationObject;
exports.createRememberLocation = createRememberLocation;
exports.getLiteralOrIdentifierName = getLiteralOrIdentifierName;
exports.getIdentifierPath = getIdentifierPath;
exports.resolveIdentifierPath = resolveIdentifierPath;
exports.rememberInterestingVariable = rememberInterestingVariable;
exports.checkInterestingPath = checkInterestingPath;
exports.createVariableDeclaratorProcessor = createVariableDeclaratorProcessor;
exports.hasUnderscore = hasUnderscore;
exports.isInteger = isInteger;
exports.endsWith = endsWith;
exports.isForbiddenObviousApi = isForbiddenObviousApi;
exports.createStorageRuleHelpers = createStorageRuleHelpers;
exports.createPropertyChecker = createPropertyChecker;
exports.createDocumentBasedRuleVisitors = createDocumentBasedRuleVisitors;
exports.findDeepestExistingPath = findDeepestExistingPath;
exports.checkAppTablesConfiguration = checkAppTablesConfiguration;
// ------------------------------------------------------------------------------
 ...
 */
function createIsWindowObject(windowObjects) {
    return function isWindowObject(node) {
        return (isWindow(node) || (!!node && isIdentifier(node) && contains(windowObjects, node.name)));
    };
}
/**
 ...
 */
function createRememberWindow(windowObjects, isWindowObject) {
    return function rememberWindow(left, right) {
        if (isWindowObject(right) && isIdentifier(left)) {
            windowObjects.push(left.name);
            return true;
        }
        return false;
    };
 ...
 */
function createIsDocument(isWindowObject) {
 ...
 */
function createIsDocumentObject(documentObjects, isDocument) {
    return function isDocumentObject(node) {
        return (isDocument(node) ||
            (!!node && isIdentifier(node) && contains(documentObjects, node.name)));
    };
}
/**
 ...
 */
function createRememberDocument(documentObjects, isDocumentObject) {
    return function rememberDocument(left, right) {
        if (isDocumentObject(right) && isIdentifier(left)) {
            documentObjects.push(left.name);
            return true;
        }
        return false;
    };
...
function buildCalleePath(node) {
    ...
    if (isMember(memberNode.object)) {
        ...
        const propertyName = isIdentifier(objectNode.property) ? objectNode.property.name : '';
        return `${buildCalleePath(memberNode.object)}.${propertyName}`;
    }
    else if (isIdentifier(memberNode.object)) {
        return memberNode.object.name;
    }
    return '';
}
/**
 ...
 */
function createIsHistory(isWindowObject) {
    return function isHistory(node) {
        if (node) {
            if (isIdentifier(node)) {
 ...
 */
function createIsHistoryObject(historyObjects, isHistory) {
    return function isHistoryObject(node) {
        return (isHistory(node) || (!!node && isIdentifier(node) && contains(historyObjects, node.name)));
    };
}
/**
 ...
 */
function createRememberHistory(historyObjects, isHistoryObject) {
    return function rememberHistory(left, right) {
        if (isHistoryObject(right) && isIdentifier(left)) {
            historyObjects.push(left.name);
            return true;
        }
        return false;
    };
 ...
 */
function createIsLocation(isWindowObject) {
    return function isLocation(node) {
        if (node) {
            if (isIdentifier(node)) {
 ...
 */
function createIsLocationObject(locationObjects, isLocation) {
    return function isLocationObject(node) {
        return (isLocation(node) ||
            (!!node && isIdentifier(node) && contains(locationObjects, node.name)));
    };
}
/**
 ...
 */
function createRememberLocation(locationObjects, isLocationObject) {
    return function rememberLocation(left, right) {
        if (isLocationObject(right) && isIdentifier(left)) {
            locationObjects.push(left.name);
            return true;
        }
        return false;
    };
 ...
 */
function getLiteralOrIdentifierName(node) {
    if (isIdentifier(node)) {
        return node.name;
    }
    else if (isLiteral(node)) {
...
function getIdentifierPath(node) {
    ...
    switch (astNode.type) {
        ...
        case 'NewExpression':
            return getIdentifierPath(node.callee);
        case 'CallExpression':
            return `${getIdentifierPath(node.callee)}().`;
        default:
            return '';
 ...
 */
function resolveIdentifierPath(path, variables) {
 ...
 * @param variables The variables lookup object
 */
function rememberInterestingVariable(node, name, variables) {
 ...
 */
function checkInterestingPath(path, interestingPathChecker) {
 ...
 */
function createVariableDeclaratorProcessor(variables, interestingPathChecker) {
    return function processVariableDeclarator(node) {
        const declaratorNode = node;
 ...
 */
function createStorageRuleHelpers(storageName, messageId) {
    return {
        /**
         ...
         */
        processVariableDeclarator(node, forbiddenStorageObjects) {
            const declaratorNode = node;
            if (declaratorNode.init) {
                if (declaratorNode.init.type === 'MemberExpression') {
                    const memberInit = declaratorNode.init;
                    const objectNode = memberInit.object;
                    const propertyNode = memberInit.property;
                ...
                else if (declaratorNode.init.type === 'Identifier' &&
                    declaratorNode.init.name === storageName) {
                    forbiddenStorageObjects.push(declaratorNode.id.name);
                }
            }
        },
         ...
         */
        checkMemberExpression(node, forbiddenStorageObjects, context) {
            ...
            if (((calleePath === storageName || calleePath === `window.${storageName}`) &&
 ...
 */
function createPropertyChecker(propertyGroups) {
    return function checkMemberExpression(node, context) {
        const memberNode = node;
        if (!memberNode.property) {
            return;
        }
        const propertyObj = memberNode.property;
        if (!('name' in propertyObj)) {
            return;
        }
        const val = memberNode.property.name;
 ...
 */
function createDocumentBasedRuleVisitors(config) {
    return function createVisitors(context) {
 ...
 */
function findDeepestExistingPath(startObject, pathSegments) {
 ...
 */
function checkAppTablesConfiguration(page, parsedApp, checkConfiguration) {
    const problems = [];
