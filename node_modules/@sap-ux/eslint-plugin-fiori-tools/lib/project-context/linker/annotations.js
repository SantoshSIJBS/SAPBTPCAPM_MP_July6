 ...
 */
function getOPTableSections(entityType, service) {
    const sections = [];
 ...
 */
function getOPHeaderSections(entityType, service) {
    const sections = [];
...
function processReferenceFacetRecord(record, aliasInfo, entityType, service, facets, index) {
    ...
    const propValues = (0, odata_annotation_core_1.elementsWithName)("PropertyValue" /* Edm.PropertyValue */, record);
    ...
    if (term === constants_1.UI_LINE_ITEM) {
        return createTableSection(facets, { index, referencedEntityType, qualifier, sectionLabel }, annotationPath, aliasInfo, service);
    }
    if (term === constants_1.UI_FIELD_GROUP) {
        return addHeaderSection(facets, { index, referencedEntityType, qualifier, sectionLabel }, annotationPath, aliasInfo, service);
    }
    return undefined;
}
/**
 ...
 * @returns
 */
function createTableSection(facets, config, annotationPath, aliasInfo, service) {
    const section = {
        type: 'table-section',
    ...
    const lineItemKey = (0, parser_1.buildAnnotationIndexKey)(config.referencedEntityType, constants_1.UI_LINE_ITEM);
 ...
 */
function addHeaderSection(headerFacets, config, annotationPath, aliasInfo, service) {
    const section = {
        type: 'header-section',
    ...
    const fieldGroupKey = (0, parser_1.buildAnnotationIndexKey)(config.referencedEntityType, constants_1.UI_FIELD_GROUP);
...
const findContentByName = (content, name) => content.find((c) => c.name === name);
const getElementText = (element) => element.content.find((c) => c.type === 'text')?.text;
/**
 ...
 */
function getTargetAnnotationPath(record) {
    const target = record.content.find((child) => {
        if (child.type === odata_annotation_core_1.ELEMENT_TYPE && child.name === "PropertyValue" /* Edm.PropertyValue */) {
            const name = (0, odata_annotation_core_1.getElementAttributeValue)(child, "Property" /* Edm.Property */);
            return name === 'Target';
        }
        return false;
    });
    if (target?.type === odata_annotation_core_1.ELEMENT_TYPE) {
        const stringAttribute = (0, odata_annotation_core_1.getElementAttribute)(target, "AnnotationPath" /* Edm.AnnotationPath */);
        if (stringAttribute) {
            return stringAttribute.value;
        }
        else {
            const annotationPathContent = findContentByName(target.content, "AnnotationPath" /* Edm.AnnotationPath */);
...
function getId(record) {
    ...
    if (id?.type === odata_annotation_core_1.ELEMENT_TYPE) {
        const stringAttribute = (0, odata_annotation_core_1.getElementAttribute)(id, "String" /* Edm.String */);
        if (stringAttribute) {
            return stringAttribute.value;
        }
        else {
            const idContent = findContentByName(id.content, "String" /* Edm.String */);
            if (idContent) {
