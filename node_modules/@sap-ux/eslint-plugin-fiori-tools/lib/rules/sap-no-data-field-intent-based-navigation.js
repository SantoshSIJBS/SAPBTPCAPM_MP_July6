 ...
 */
function getIntentBasedNavDataFields(tableOrFieldGroup, aliasInfo) {
    if (!tableOrFieldGroup.annotation) {
        return [];
    }
    let collection;
    if (tableOrFieldGroup.type === 'table') {
        [collection] = (0, odata_annotation_core_1.elementsWithName)("Collection" /* Edm.Collection */, tableOrFieldGroup.annotation.annotation.top.value);
    }
    else {
        const [record] = (0, odata_annotation_core_1.elementsWithName)("Record" /* Edm.Record */, tableOrFieldGroup.annotation.annotation.top.value);
        const [propertyValue] = (0, odata_annotation_core_1.elements)((el) => el.name === "PropertyValue" /* Edm.PropertyValue */ && el.attributes["Property" /* Edm.Property */]?.value === 'Data', record);
        [collection] = (0, odata_annotation_core_1.elementsWithName)("Collection" /* Edm.Collection */, propertyValue);
    }
    if (!collection) {
        return [];
    }
    const records = (0, odata_annotation_core_1.elements)((element) => {
        ...
        const recordType = (0, annotations_1.getRecordType)(aliasInfo, element);
        return (recordType === 'com.sap.vocabularies.UI.v1.DataFieldForIntentBasedNavigation' ||
 ...
 */
function checkTablesAndFieldGroupsInPage(page, parsedService, problems) {
    for (const tableOrFieldGroup of [...(page.lookup['table'] ?? []), ...(page.lookup['field-group'] ?? [])]) {
        ...
        itentBasedNavigationDataFields.forEach((dataField) => {
            ...
            const alreadyReportedDFIndex = problems.findIndex((problem) => problem.annotation.reference.value === dataField);
            if (alreadyReportedDFIndex > -1) {
                ...
                problems.push({
                    type: diagnostics_1.NO_DATA_FIELD_INTENT_BASED_NAVIGATION,
                    ...
                    annotation: {
                        ...
                        recordType: (0, annotations_1.getRecordType)(aliasInfo, dataField) ?? '',
...
const rule = (0, rule_factory_1.createFioriRule)({
    ruleId: diagnostics_1.NO_DATA_FIELD_INTENT_BASED_NAVIGATION,
    meta: {
        type: 'suggestion',
        docs: {
            recommended: true,
            description: 'UI.DataFieldForIntentBasedNavigation and UI.DataFieldWithIntentBasedNavigation must not be used.',
        ...
        messages: {
            ['no-data-field-for-intent-based-navigation']: 'DataFieldForIntentBasedNavigation annotation must not be used. Please use a semantic link navigation instead.',
            ['no-data-field-with-intent-based-navigation']: 'DataFieldWithIntentBasedNavigation annotation must not be used. Please use a semantic link navigation instead.'
        }
    },
    check(context) {
        const problems = [];
        ...
        return {
            ['target>element[name="Annotation"]'](node) {
                ...
                validationResult
                    ...
                    context.report({
                        node: dfNode,
                        messageId: result.annotation.recordType ===
...
//# sourceMappingURL=sap-no-data-field-intent-based-navigation.js.map