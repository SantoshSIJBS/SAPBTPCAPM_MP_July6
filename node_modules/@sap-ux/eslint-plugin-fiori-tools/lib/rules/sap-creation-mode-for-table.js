 ...
 */
function reportDiagnostic(problems, { messageId, pageName, pageSectionName, parsedApp, configurationPath, tableType, validValues = [], recommendedValue }) {
    problems.push({
 ...
 */
function validateCreateModeV2(createMode, pageName, parsedApp, tableType, problems, pageSectionName) {
 ...
 * @param pageSectionName - Name of the object page section where the issue occurs
 */
function processTableV2(table, page, appCreateMode, parsedApp, problems, pageSectionName) {
 ...
 */
function validateCreationModeV4(creationMode, page, parsedApp, tableType, recommendedValue, problems, shouldSuggestAppLevel) {
 ...
 * @param pageSectionName - Name of the object page section where the issue occurs
 */
function processTableV4(table, page, appCreateMode, parsedApp, problems, shouldSuggestAppLevel, pageSectionName) {
    ...
    // Check table level
    if (validateCreationModeV4(tableCreationMode, { name: page.targetName, sectionName: pageSectionName }, parsedApp, tableType, recommendedValue, problems, shouldSuggestAppLevel)) {
        return;
    }
    // Check app level
    ...
    for (const [appKey, app] of Object.entries(context.sourceCode.projectContext.linkedModel.apps)) {
        ...
        for (const page of app.pages) {
            ...
            for (const tableSection of page.sections.filter((section) => section.type === 'table-section')) {
                const table = tableSection.children.find((element) => element.type === 'table');
                if (table) {
                    processTableV4(table, page, appCreateMode, parsedApp, problems, shouldSuggestAppLevel, tableSection.annotation?.label);
                }
            }
        }
    }
...
const rule = (0, rule_factory_1.createFioriRule)({
    ...
    meta: {
        ...
        messages: {
            invalidCreateMode: 'Invalid createMode value: "{{value}}"{{tableType}}{{sectionText}}. The recommended value is "creationRows".{{validValues}}',
            ...
            invalidCreateModeV4: 'Invalid creationMode value "{{value}}"{{tableType}}{{sectionText}}. The recommended value is "{{recommendedValue}}".{{validValues}}',
